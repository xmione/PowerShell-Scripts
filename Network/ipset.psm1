...
function Remove-HostNetworkAdapter {
    ...
    try {
        ...
        if ($adapter) {
            Write-Output "Network adapter Interface Description'$adapter.InterfaceDescription '."
...
function Set-VM-Network-Adapter {
    param (
        [string]$VMName = "Windows 11 dev environment",
        [string]$InternalSwitchName = "Internal",
        [string]$HostBridgedAdapterName = "vEthernet (Bridged Network)",
        [string]$HostInternalAdapterName = "vEthernet (Internal)",
        [string]$HostInternalIP = "192.168.100.74",
        [string]$VMIPAddress = "192.168.100.75",
        [int]$VMSubnetMask = 24,
        [string]$DNS = "8.8.8.8",
    ...
    $SecurePassword = ConvertTo-SecureString $env:VMPassword -AsPlainText -Force
    ...
    Write-Output "Static IP assignment complete."
     ...
     if ($hostBridgedAdapter -and $hostInternalAdapter) {
         if ($hostBridgedAdapter.Status -eq 'Up') {
             ...
             New-NetFirewallRule -DisplayName "Allow Inbound Traffic on $HostInternalAdapterName" -Direction Inbound -InterfaceAlias $HostInternalAdapterName -Action Allow
 
             # Add outbound firewall rule
             New-NetFirewallRule -DisplayName "Allow Outbound Traffic on $HostInternalAdapterName" -Direction Outbound -InterfaceAlias $HostInternalAdapterName -Action Allow
...
# Call the function
# Add-VMFirewallRule
function Add-VMFirewallRule {
    param (
        [string]$NetworkName = "vEthernet (Internal)"
    )

    # Get the network adapter
    $networkAdapter = Get-NetAdapter | Where-Object { $_.Name -eq $NetworkName }
    if (-not $networkAdapter) {
        Write-Host "Network adapter $NetworkName not found."
        return
    }
    ...
    $inboundRuleExists = Get-NetFirewallRule | Where-Object {
        $_.DisplayName -eq "Allow Inbound Traffic on $NetworkName" -and $_.Direction -eq "Inbound" -and $_.InterfaceAlias -eq $interfaceAlias
    }

    # Check for existing outbound rule
    $outboundRuleExists = Get-NetFirewallRule | Where-Object {
        $_.DisplayName -eq "Allow Outbound Traffic on $NetworkName" -and $_.Direction -eq "Outbound" -and $_.InterfaceAlias -eq $interfaceAlias
    }

    # Add inbound rule if it doesn't exist
    if (-not $inboundRuleExists) {
        New-NetFirewallRule -DisplayName "Allow Inbound Traffic on $NetworkName" -Direction Inbound -InterfaceAlias $interfaceAlias -Action Allow
        Write-Host "Inbound firewall rule added for $NetworkName."
    } else {
        Write-Host "Inbound firewall rule already exists for $NetworkName."
    }

    # Add outbound rule if it doesn't exist
    if (-not $outboundRuleExists) {
        New-NetFirewallRule -DisplayName "Allow Outbound Traffic on $NetworkName" -Direction Outbound -InterfaceAlias $interfaceAlias -Action Allow
        Write-Host "Outbound firewall rule added for $NetworkName."
    } else {
        Write-Host "Outbound firewall rule already exists for $NetworkName."
    }
}

Export-ModuleMember -Function Get-NetworkAdapterSettings, Set-NetworkAdapterSettings, Remove-HostNetworkAdapter, Remove-VMNetworkAdapter, Disable-HostNetworkAdapter, Get-VMNetworkAdapters, Set-VM-Network-Adapter, Add-VMFirewallRule
